...
execute if entity @s at @s run tellraw @a[tag=eoflib.debug, scores={eoflib.debug_mode=3..}, distance=0.1..] [{"text": "[", "color": "dark_gray"}, {"text": "Debug", "color": "gold"}, {"text": "] - ", "color": "dark_gray"}, {"text": "eof:ability_data/rainwings.mcf", "color": "#967228", "hoverEvent": {"action": "show_text", "value": [{"selector": "@s", "color": "aqua"}, {"text": " ran the following function:\n- eof:load/gen/storages/ability_data/rainwings.mcfunction", "color": "aqua"}]}}]
...
data modify storage eoflib:config eof.Abilities[].RainWings.primary[0].eof_data set value { \
    ...
    "raycast_data": { \
        "arc_intensity": 0, \
        "range": 14, \
        "particle": 'dust{"color": [0.0, 0.0, 0.0], scale: 1.25}', \
        "blocks": { \
